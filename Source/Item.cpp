...
Item::Item(const std::string& blueprintName, const WorldCoords2D& worldPosition)
{
	s_ID++;
	m_itemID = s_ID;
	m_worldPosition = worldPosition;
	std::map<std::string, ItemBlueprint*>::iterator bpIter;
	bpIter = ItemBlueprint::s_registeredItemBlueprint.find( blueprintName );
	m_isVBODirty = true;
	m_numVertex = 0;
	m_vboID = 0;
	 
	if( bpIter != ItemBlueprint::s_registeredItemBlueprint.end() )
	{
		ItemBlueprint* blueprint = bpIter->second;
		m_itemBlueprint = blueprint;
		PopulateFromBlueprint( *blueprint );
	}
	else
	{
		DebuggerPrintf( "Cannot Find Item with name %s.\n", blueprintName.c_str() );
		exit(0);
	}
}
...
void Item::PopulateFromBlueprint(const ItemBlueprint& blueprint)
{
	m_damage = blueprint.m_damage;
...
void Item::CreateVBO()
{
	...
	itemTopLeftTexCoords.x = (float)m_itemBlueprint->m_textureTileCoords.x * texFrameWidthX;
	itemTopLeftTexCoords.y = (float)m_itemBlueprint->m_textureTileCoords.y * texFrameHeightY;
